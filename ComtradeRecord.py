...
class ComtradeRecord(object):
    """
    可以读取IEEE标准电力系统暂态数据交换通用格式COMTRADE 的Class.
    """
    ...
    def get_timestamps(self):
        ...
        for nr in range(self.cfg_data['nrates']):
            ...
            if (nr != 0):
                num = n_samples - self.cfg_data['endsamp'][nr - 1]
            else:
                num = n_samples
            # 划分分段结束时间
            t_end = t_start + (num - 1) * t_interval
            tempstamps = np.linspace(t_start, t_end, num)
    ...
    def read_ascii(self, file_path):
        ...
        # Open data
        data = pd.read_csv(file_path, index_col=0, header=None, encoding='utf-8')
